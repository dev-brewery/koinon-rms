/**
 * Families Import Page
 * Multi-step wizard for importing family data from CSV
 */
...
import type { TargetField } from '@/components/import/targetFields';

// Family-specific target fields
...
export function FamiliesImportPage() {
  ...
  const executeMutation = useMutation({
    mutationFn: async () => {
      ...
      return executeImport(file, 'Families', mappingsRecord);
    },
    onSuccess: (data) => {
      setImportJob(data);
      goToStep('progress');
  ...
  const canProceedToValidation = () => {
    const requiredFields = FAMILY_TARGET_FIELDS.filter((f) => f.required);
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
          ...
          <span>/</span>
          <span className="text-gray-900">Families Import</span>
        </div>
        <h1 className="text-3xl font-bold text-gray-900">Import Families</h1>
        <p className="mt-2 text-gray-600">
          Upload and map CSV data to import family records
        </p>
      </div>
      ...
      {currentStep === 'mapping' && (
        <div className="space-y-4">
          <FieldMappingEditor
            ...
            targetFields={FAMILY_TARGET_FIELDS}
