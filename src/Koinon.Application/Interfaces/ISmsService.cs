...
public interface ISmsService
{
    ...
    /// <summary>
    /// Sends an MMS message with media attachments and returns the provider message ID (Twilio SID).
    /// </summary>
    /// <param name="toPhoneNumber">The destination phone number in E.164 format (e.g., +15551234567)</param>
    /// <param name="message">The message body to send</param>
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Result containing success status, message ID, and error details if failed</returns>
    Task<SmsResult> SendMmsAsync(string toPhoneNumber, string message, IEnumerable<string> mediaUrls, CancellationToken cancellationToken = default);
...
/// <param name="SegmentCount">Number of message segments sent (for cost tracking)</param>
public record SmsResult(bool Success, string? MessageId, string? ErrorMessage, int SegmentCount = 1);
