...
let cachedGraphBaseline;
// Load and cache graph baseline
function loadGraphBaseline() {
...
function validateDatabaseNaming(names) {
    const issues = [];
...
function validateCSharpNaming(names) {
    const issues = [];
...
function validateTypeScriptNaming(names) {
    const issues = [];
...
function validateDependencies(project, dependencies) {
    const issues = [];
...
function getArchitectureGuidance(topic) {
    const guidance = {
        'entity': `
...
function getControllerPattern(entityName) {
...
function getEntityChain(entityName) {
    const baseline = loadGraphBaseline();
    // Find entity
    ...
    return {
        ...
        service: service ? {
            ...
            methods: service.methods.map((m) => ({ name: m.name, return_type: m.return_type }))
        } : null,
...
function validateNewController(name) {
    const issues = [];
    const warnings = [];
...
function analyzeFileImpact(filePath) {
    ...
    if (fileAnalysis.layer === 'Domain' && fileAnalysis.entityName) {
        ...
        const linkedDtos = Object.entries(baseline.dtos).filter(([_, dto]) => dto.linked_entity === entityName);
        ...
        const linkedServices = Object.entries(baseline.services).filter(([_, service]) => {
            const methods = service.methods || [];
            return methods.some((m) => m.return_type?.includes(entityName) ||
                m.parameters?.some((p) => p.type?.includes(entityName)));
        });
        ...
        const linkedControllers = Object.entries(baseline.controllers).filter(([_, controller]) => {
            const endpoints = controller.endpoints || [];
            return endpoints.some((e) => linkedDtos.some(([dtoName]) => e.response_type?.includes(dtoName)));
        });
        ...
        const linkedControllers = Object.entries(baseline.controllers).filter(([_, controller]) => {
            const endpoints = controller.endpoints || [];
            return endpoints.some((e) => e.response_type?.includes(dtoName) || e.request_type?.includes(dtoName));
        });
        for (const [controllerName] of linkedControllers) {
...
function findFrontendConnections(baseline, entityName, linkedDtos) {
    const connections = [];
    ...
    const relevantApiFunctions = Object.values(baseline.api_functions || {}).filter((fn) => dtoNames.some(dto => fn.responseType?.includes(dto)));
    for (const fn of relevantApiFunctions) {
        ...
        const hookEdges = (baseline.edges || []).filter((e) => e.target === fn.name && baseline.hooks?.[e.source]);
        for (const edge of hookEdges) {
            ...
            const componentEdges = (baseline.edges || []).filter((e) => e.target === edge.source && baseline.components?.[e.source]);
...
function findFrontendConnectionsForDto(baseline, dtoName) {
    const connections = [];
    const relevantApiFunctions = Object.values(baseline.api_functions || {}).filter((fn) => fn.responseType?.includes(dtoName));
    for (const fn of relevantApiFunctions) {
        connections.push({
            path: `src/web/src/${fn.path}`,
            layer: 'Frontend',
        ...
        for (const edge of hookEdges) {
            ...
            const componentEdges = (baseline.edges || []).filter((e) => e.target === edge.source);
            for (const cEdge of componentEdges) {
                connections.push({
                    path: `src/web/src/components/${cEdge.source}.tsx`,
                    layer: 'Frontend',
...
function findFrontendConnectionsForController(baseline, controllerName) {
    const connections = [];
    const resourceName = controllerName.replace('Controller', '');
    const relevantApiFunctions = Object.values(baseline.api_functions || {}).filter((fn) => fn.endpoint?.includes(resourceName.toLowerCase()));
    for (const fn of relevantApiFunctions) {
        connections.push({
            path: `src/web/src/${fn.path}`,
            layer: 'Frontend',
